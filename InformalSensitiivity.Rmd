...
```{r solar}
source(here::here("solarpv.R"))
solarpv

# read in R formatted data
#load("../Data/sierraczosolar.rda")
load(here::here("sierraczosolar.rda"))
...
varying efficiency aka `ethresh` (here it will be revenue for us)
...
# making a list of efficiency (revenue) inputs
eff = rnorm(mean=0.6, sd = 0.1, n=20)

# site2: this outputs a list of lists with the output from a range of parm inputs
# eff = .x take the value that I piped in, kinda like indexing to the efficiency?
# map is kinda iterating the solarpv function over the different efficiency values 
## WHERE TF IS YEAR?? WHAT IS HAPPENING??

## for each eff she pulls in the year and the mean

site2 = eff %>% 
  map(~solarpv( area=0.1, solar=sierraczosolar, clr="green", eunit="W", g=FALSE, etype="direct", eff=.x )) 

head(site2) # this is a list of lists of stuff...hard to get info out of this
...
#map_df is a function that returns a dataframe
# `[` (note the backticks!) is a function that implements the subsetting operation
# c("annual") subset the "annual" from site2

# now make it a dataframe that humans would like to use, pull the year by taking the column "annual$year" from the tmp dataframe, and similar for electricity
site2df = data.frame(year = tmp$annual$year, elect= tmp$annual$elect)

# now we could plot
## box shows parameter uncertainty
## why do this? to see how big the range of output might be relative to "uncertain" parameter range
...
## this gives a list of average for each year
site2_average = site2df %>% group_by(year) %>% 
