...
-   some things to think about that might help make it easier to "see" differences betwee observed time series and modeled time series
...
Looks like `obs` is actual streamflow and `model` and the units are mm/day, and we are responsible for determining how we want to analyze the difference in the obs and model as the error

First, we need to clean the data and plot the model's predictions along with the actual data over:

overall timeseries

smaller timeline

observed vs model (x vs y) to see patterns of the model's error in the predictions
...
Especially if you want to rank different models or create indices like Sobol Sensitivity Indices
...
Example - a dataset where each column is a different model run for Sagehen Creek (using different parameters)
